...
async function extractPage(page: Page, pageNumber: number, pageUrl: string, resolved: Resolved | null, fromIndex: number): Promise<Extraction> {
  ...
  for (const { field, missingRows } of states) {
    ...
    const missing = rows.length === 0 || missingRows.length === rows.length;
    if (missing && rows.length > 0) missingRequired.push(field.name);
    if (!missing && missingRows.length > 0) {
      warnings.push(
        'required field "' + field.name + '" missing on page ' + pageNumber + ' row' + (missingRows.length > 1 ? 's ' : ' ') + missingRows.join(', '),
      );
    }
  }
...
async function scrape(opts: Options): Promise<number> {
  ...
  try {
    ...
    for (;;) {
      ...
      const summary: PageSummary = { page: pageNumber, url, firstKey: keys[0] ?? null, raw: extraction.rows.length, kept: kept.length };
